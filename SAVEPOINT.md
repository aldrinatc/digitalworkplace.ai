...
**Current savepoint — 2026-09-16, 17:48 Asia/Dubai:** Work paused and project browser tabs closed at Aldrin's request. Resume from [the recovery savepoint](docs/incidents/2026-09-16-resume-savepoint.md) and [latest auth verification](docs/incidents/2026-09-16-auth-verification.md). All six public sites passed at 17:45; existing main/Support/Chat Core sessions survived refresh and protected APIs rejected unauthorized requests. The main new-user/profile/admin repair remains undeployed because its server credential is still pending. Production remains live. Existing hourly monitoring remains active, with feature work paused. The expanded 19-probe monitor passed ten tests and is backed up separately on `codex/site-uptime-monitor` at `e71950f`; its proposed 15-minute cloud schedule is NOT activated. Do not interpret availability or existing-session checks as full new-user auth certification.
