...
**Current savepoint — 2026-09-16, 18:08 Asia/Dubai:** Operational monitoring resumed at Aldrin's request. All six sites plus auth/AI/database checks passed in the cloud (25 probes). The expanded 15-minute monitor is now published on work-fork `main` at `d8a0795`; two manual cloud runs passed, but the first automatic scheduled run remains unverified. The existing Codex incident-response heartbeat is ACTIVE every 15 minutes. All six Vercel projects have Fluid Compute and elastic concurrency enabled; capacity settings were reviewed, not stress-tested. Main new-user/profile/admin repair remains undeployed pending its server credential. Read [the recovery record](docs/incidents/2026-09-16-resume-savepoint.md) and [operations runbook](docs/operations/production-on-call.md) before changes. No production app was redeployed during this monitoring activation.
