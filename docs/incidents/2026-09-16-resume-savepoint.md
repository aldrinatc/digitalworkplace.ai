...
- **Automatic schedule fault OPEN — MON-20260916-01:** the 14:51 UTC heartbeat detected cloud evidence older than 45 minutes and no schedule-event runs despite active/default-branch configuration. Manual fallback run `35111394084` passed all 25 probes at 14:51 UTC. Track recurring schedule execution separately from this fresh manual result; do not close the fault until a real schedule-event production-health run succeeds. Suppress repeated unchanged alerts.
